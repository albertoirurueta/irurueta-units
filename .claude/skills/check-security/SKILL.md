...
Run `command -v detect-secrets` (or `detect-secrets --version`). If it succeeds, skip to Step 3.

## Step 2 — Attempt installation if missing
...
- **Argument provided** (a path or glob): scope the scan to it, ad hoc, without touching any baseline file — go
  to Step 5.
- **No argument**: scope to the whole repository using the baseline workflow — go to Step 4.

## Step 4 — Whole-repository scan via baseline
...
- **No `.secrets.baseline` at the repository root**: create one with `detect-secrets scan > .secrets.baseline`.
  Every finding in a freshly created baseline is "new" by definition — report all of them (Step 6).
...
Go to Step 6 to report.

## Step 5 — Scoped ad hoc scan
...
State plainly whether the scan ran at all (per Steps 1–2). If it did, report:
