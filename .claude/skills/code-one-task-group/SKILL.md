...
back-and-forth with the user as possible. This is the execution unit the `code` skill calls once per group; it
carries the dispatch and progress-tracking logic that used to live in `code`'s own Step 4/Step 5, now scoped to
...
`find .claude/skills -maxdepth 1 -type d -name "*-code-one-task-group"`. The directory name minus the
`-code-one-task-group` suffix is the key (e.g. `java-code-one-task-group` → `java`).
...
unblocked tasks silently; surface it as part of this skill's own report so the caller (`code`) can decide whether
...
- Add license headers to touched files if this repository has a convention for them — delegate to the
  `gate-runner` agent rather than running `check-license` directly if that skill is installed:
  `Agent({description: "Add license headers for <file(s)>", subagent_type: "gate-runner", prompt: "Invoke
...
- Keep documentation comments current for the language in play, using whichever doc-comment skill this
  repository has installed for it, the same way, via `gate-runner`.
- Run the relevant tests and check coverage for the touched code, and check for new code-quality issues against
  a pre-change baseline you capture yourself before implementing — all via `gate-runner` delegating to whichever
...
achieved, and code-quality outcome (or blocker detail if it didn't finish). This is what the caller (`code`'s own
