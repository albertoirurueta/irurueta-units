...
1. **Run exactly what the prompt asks**, scoped exactly as instructed (a specific class/type/file selector, or
   ...
   - Prefer invoking the named Claude Code skill via `Skill({skill: "<name>", args: "<...>"})` — this repository's
     skills (`java-test`, `dotnet-test`, `java-coverage`, `dotnet-coverage`, `java-code-quality`,
     `dotnet-code-quality`, `check-license`, `java-javadoc`, `dotnet-docfx`, `check-security`, or equivalents in
...
3. **If the invoked skill itself needs a decision only a person can make** (e.g. `check-license`'s choice of
   whether to generate a header convention from scratch, `check-security`'s audit-labeling step), let it surface
