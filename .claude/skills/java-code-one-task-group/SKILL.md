---
name: java-code-one-task-group
description: Implement one bucket of Java tasks from a plan's task group end-to-end — captures a single pre-change quality baseline for the whole bucket, runs `java-code-one-task` once per task (in parallel agents when the group is marked parallelizable) to implement each one and its tests, then validates the entire bucket once: license headers, Javadoc, scoped tests, an 80% coverage check, a full-suite run, and a code-quality regression check against that one baseline. Checks off each task/sub-task's box in `implementation_plan.md` and notifies the user as each phase completes — task implementation first, then group validation — rather than waiting until the whole bucket is done. If group validation surfaces a regression traceable to a specific task, that task's implementation is revised and the affected checks re-run, instead of re-validating the whole bucket per task. Invoke as `/java-code-one-task-group <bucket text>`, passing every task/sub-task in the bucket, each with its own text, plus the group's `Parallelizable` verdict and any relevant "Current code state" context. Equivalent to `dotnet-code-one-task-group` for Java/Maven projects; used by `code-one-task-group`, which invokes this once per plan group for its Java-tagged tasks.
model: sonnet
...
per-task validation used to spend redundantly. It is the Java/Maven counterpart to `dotnet-code-one-task-group` —
same shape, but built on `java-code-quality`, `java-test`, `java-coverage`, and `java-javadoc`. It uses a medium
...
the `gate-runner` agent rather than running `java-code-quality` directly: `Agent({description: "Capture
...
for these classes."})`. `gate-runner` runs in its own separate context and reports back just the issue list,
...
capture if the `java-code-quality` skill is unavailable in this repository.

## Step 2 — Implement each task via `java-code-one-task`

For every task in the bucket, invoke `java-code-one-task` — it implements exactly what the task specifies and
writes/updates its tests, nothing more (license headers, Javadoc, and all validation now live here instead).

- **Bucket marked `Parallelizable: yes`**: invoke `java-code-one-task` for every task in the bucket concurrently
...
2. **Javadoc**, for every class any task in the bucket added or modified, by delegating to `gate-runner`:
   `Agent({description: "Update Javadoc for task group", subagent_type: "gate-runner", prompt: "Invoke
   ...
   and re-invoke until it reports success. Skip this item if `java-javadoc` is unavailable in this repository.
3. **Scoped tests**, for every class affected across the bucket, by delegating to `gate-runner`: `Agent({description:
   "Run tests for task group", subagent_type: "gate-runner", prompt: "Invoke Skill({skill: \"java-test\", args:
...
4. **Coverage**, for every changed/new class across the bucket, by delegating to `gate-runner`: `Agent({description:
   "Check coverage for task group", subagent_type: "gate-runner", prompt: "Invoke Skill({skill: \"java-coverage\",
...
5. **Full suite**, to catch regressions this bucket's changes may have caused elsewhere, by delegating to
   `gate-runner`: `Agent({description: "Run full test suite", subagent_type: "gate-runner", prompt: "Run `mvn
...
6. **Code quality**, for the same class set as Step 1, by delegating to `gate-runner`: `Agent({description: "Check
   ...
   `java-code-quality` is unavailable in this repository.
