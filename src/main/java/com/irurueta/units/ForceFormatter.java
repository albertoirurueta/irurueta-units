...
public class ForceFormatter extends MeasureFormatter<Force, ForceUnit> {
    ...
    /**
     * Formats and converts provided force value and unit using metric unit
     * system.
     * If provided force value is too large for provided force unit,
     * this method will convert it to a more appropriate unit.
     *
     * @param value a force value.
     * @param unit  a force unit.
     * @return a string representation of force value and unit using metric
     * unit system.
     */
    public String formatAndConvertMetric(final Number value, final ForceUnit unit) {
        final var v = value.doubleValue();

        final var dyne = ForceConverter.convert(v, unit, ForceUnit.DYNE);
        if (Math.abs(dyne) < 1.0 / ForceConverter.NEWTONS_PER_DYNE) {
            return format(dyne, ForceUnit.DYNE);
        }

        final var newton = ForceConverter.convert(v, unit, ForceUnit.NEWTON);
        if (Math.abs(newton) < ForceConverter.NEWTONS_PER_KILOGRAM_FORCE) {
            return format(newton, ForceUnit.NEWTON);
        }

        final var kilogramForce = ForceConverter.convert(v, unit, ForceUnit.KILOGRAM_FORCE);
        if (Math.abs(kilogramForce) < ForceConverter.NEWTONS_PER_KILONEWTON
                / ForceConverter.NEWTONS_PER_KILOGRAM_FORCE) {
            return format(kilogramForce, ForceUnit.KILOGRAM_FORCE);
        }

        final var kilonewton = ForceConverter.convert(v, unit, ForceUnit.KILONEWTON);
        return format(kilonewton, ForceUnit.KILONEWTON);
    }

    /**
     * Formats and converts provided force value and unit using imperial unit
     * system.
     * Since pound-force is the only supported imperial force unit, provided
     * value is always converted to it.
     *
     * @param value a force value.
     * @param unit  a force unit.
     * @return a string representation of force value and unit using imperial
     * unit system.
     */
    public String formatAndConvertImperial(final Number value, final ForceUnit unit) {
        final var v = value.doubleValue();

        final var poundForce = ForceConverter.convert(v, unit, ForceUnit.POUND_FORCE);
        return format(poundForce, ForceUnit.POUND_FORCE);
    }
