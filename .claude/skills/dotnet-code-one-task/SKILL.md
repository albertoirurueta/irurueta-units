...
narrowest execution unit in the `code` pipeline: `dotnet-code-one-task-group` calls it once per task in a bucket
...
run the test suite yourself — `dotnet-code-one-task-group` runs it once for the whole bucket in its own
...
this task stopped on a blocker per Step 4. This is what `dotnet-code-one-task-group` records for this task before
