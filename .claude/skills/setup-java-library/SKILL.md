...
This skill can be invoked stand-alone (`/setup-java-library`) or as a step inside another skill, such as
`setup-java-library-repository`, which resolves these same inputs itself (e.g. after asking the user once, or
...
file at the repository root if one doesn't exist yet — the `check-license` skill can verify/backfill file headers
...
file. If a license was chosen and no `LICENSE` file exists yet, suggest the `check-license` skill to generate one
